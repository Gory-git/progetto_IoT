...
qemu-system-mips -m 256 -M malta -kernel ${KERNEL} -drive if=ide,format=raw,file=${IMAGE} -append "firmadyne.syscall=1 root=/dev/sda1 console=ttyS0 nandsim.parts=64,64,64,64,64,64,64,64,64,64 rdinit=/firmadyne/preInit.shrw debug ignore_loglevel print-fatal-signals=1" -serial stdio -monitor none -display none -nographic -net nic,model=rtl8139 -net tap,ifname=qemu-tap0,script=no,downscript=no
