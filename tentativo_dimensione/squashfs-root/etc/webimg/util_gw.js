...
var ws_w_wpa2=4;
var ws_w_mix=6;
var Def_Main_Page="home.htm";


var submitTimeout;
function showWlResponse(responseText, statusText, xhr, $form)
{
	if (statusText == "success")
	{
		//alert(responseText)
		clearTimeout(submitTimeout);
		$("#loginimg").attr("class", "off");
	}
}
...
function jq_submit(url,value,show)
{
	var JqTimeout = 6000;
	
	$("#submit-url").val(url);
	$("#submit-value").val(value);
	if (show)
		$("#loginimg").attr("class", "on");
	submitTimeout = setTimeout(TimeOut, JqTimeout);
	var option = { 		
		success:       showWlResponse,
		//beforeSubmit:  showRequest,
		timeout:       JqTimeout
	};
	$("#idFSetup").ajaxSubmit(option); 
	return false;
}
...
function AllDiv(com, div)
{
	for (var i=0;i<div.length;i++)
		show_div(com, div[i]);	
}

function DoDiv(num,div)
{
	AllDiv(false, div);	

			show_div(true, div[num]);
...
function CheckNull(I)
{
	...
	{
		if (get_by_id(I[i]).value=="" || get_by_id(I[i]).value.length==0)
		{
			alert(errmsg.msg2);
			get_by_id(I[i]).focus();
			return false
...
function Value_Add_IP(I)
{
	return get_by_id(I[0]).value+"."+get_by_id(I[1]).value+"."+get_by_id(I[2]).value+"."+get_by_id(I[3]).value;
}
function valid_range(I,start,end) {

	d = parseInt(I.value, 10);
	if ( !(d<=end && d>=start) ) {
		alert(errmsg.msg1+"[" + start + " - " + end +"].");
...
function hidethisbtn(id)
{
	get_by_id(id).disabled=true;
}
function submit(F,url,value)
{
	get_by_id("submit-url").value=+url;
	get_by_id("submit-value").value=value;
	F.submit();
}

function submit_noreset(F,url,value)
{
	get_by_id("submit-url").value=url;
	get_by_id("submit-value").value=value;
	F.submit();
...
function saveChanges_wan(form , MultiPppoeFlag, dynamicWanIP)
{
  ...
  if ( wanType == 0 ){ //static IP
	  if ( checkIpAddr(form.wan_ip, tz.alert3) == false )
	    return false;
	  ...
	  if (form.wan_gateway.value!="" && form.wan_gateway.value!="0.0.0.0") {
	    ...
	    if ( checkIpAddr(form.wan_gateway, inet.alert1) == false )
	      return false;
	  ...
	  if (form.fixedIpMtuSize != null){
	   ...
	   if(MultiPppoeFlag ==1)
	 	   if (ppp_checkSubNetFormat(form.pppSubNet_1,inet.alert7) == false)
	  			return false; 
	  ...
	  //----------------------Second pppoe info check  Begin----------------------------- 
	   if(pppoeNumber >= 1)
	   {
		   ...
		   if (ppp_checkSubNetFormat(form.pppSubNet_2,inet.alert7) == false)
  		   		return false; 
...
//----------------------Third pppoe info check  Begin--------------------------------	
	  ...
	  if(pppoeNumber >= 2)	
	  {
		   ...
		   if (ppp_checkSubNetFormat(form.pppSubNet_3,inet.alert7) == false)
  		   		return false; 		   
	  }
...
function unmask(checked,sp,id,name)
{
	...
	//alert(tmp);
	if(checked)
		get_by_id(sp).innerHTML='<input type="text" name="'+name+'" id="'+id+'" size="25" maxlength="64" onkeyup="CountLeng(this, \'u_ms\')" value="'+tmp+'">';
	else
		get_by_id(sp).innerHTML='<input type="password" name="'+name+'" id="'+id+'" size="25" onkeyup="CountLeng(this, \'u_ms\')" maxlength="64" value="'+tmp+'">';
	
}
function unmask1(checked,sp,id,name)
{
	...
	//alert(tmp);
	if(checked)
		get_by_id(sp).innerHTML='<input type="text" name="'+name+'" id="'+id+'" size="25" maxlength="64" onkeyup="CountLeng(this, \'c_p\')" value="'+tmp+'">';
	else
		get_by_id(sp).innerHTML='<input type="password" name="'+name+'" id="'+id+'" size="25" onkeyup="CountLeng(this, \'c_p\')" maxlength="64" value="'+tmp+'">';
...
function CtlDisabled(en , div)
{
	for (var i=0; i<div.length; i++)
		get_by_id(div[i]).disabled = en;
}
...
function changlang(idx)
{
	//alert(idx);
	get_by_id("lang_idx").value=idx;
	document.langeage.submit();
}
...
function disable(I,n,off)
{
	for (i=0;i<n;i++)
	{
		if (off)
			choose_enable(get_by_id(I[i]));
		else
			choose_disable(get_by_id(I[i]));
	}
}
